...
{
	if [ ! -z $4 ]; then
		SERVER_ALIAS="ServerAlias $4"
	else
		SERVER_ALIAS="#ServerAlias your.alias.here"
	fi
	
	if [ -n $5 ]; then
		SERVER_ALIAS="$SERVER_ALIAS $5"
	fi
...
#NPA : ne pas vérifier les nouvelles versions
#version_check
...
#
if [ ! -z $DELETE ]; then
	/bin/echo -n "Entrer le nom du host à supprimer [${VIRTUALHOST}.loc]: "
	read WILDCARD_ZONE

	if [ -z $WILDCARD_ZONE ]; then
		WILDCARD_ZONE="$VIRTUALHOST.loc"
	fi

	if host_exists $WILDCARD_ZONE ; then
		/bin/echo -n "- Deleting virtualhost, $WILDCARD_ZONE... Continue? [Y/n]: "
		read continue
		case $continue in
		n*|N*) exit
		esac

		/bin/echo -n "  - Removing $WILDCARD_ZONE from /etc/hosts... "
		cat /etc/hosts | grep -v $WILDCARD_ZONE > /tmp/hosts.tmp
	...
	/bin/echo -n "Supprimer un autre host? [laisser vide pour non]: "
	read ADDITIONAL_HOST

	if [ -n $ADDITIONAL_HOST ]; then
		if host_exists $ADDITIONAL_HOST ; then
			/bin/echo -n "  - Removing $ADDITIONAL_HOST from /etc/hosts... "
			cat /etc/hosts | grep -v $ADDITIONAL_HOST > /tmp/hosts.tmp
			if [ -s /tmp/hosts.tmp ]; then
				mv /tmp/hosts.tmp /etc/hosts
			fi
			/bin/echo "done"
...
#Choix du host à créer
/bin/echo -n "Entrer le nom du host [${VIRTUALHOST}.loc]: "
...
if ! checkyesno ${SKIP_ETC_HOSTS}; then
	if ! host_exists $WILDCARD_ZONE ; then
		...
		/bin/echo "Creating a virtualhost for $WILDCARD_ZONE..."
		/bin/echo -n "+ Adding $WILDCARD_ZONE to /etc/hosts... "
		/bin/echo "$IP_ADDRESS	$WILDCARD_ZONE" >> /etc/hosts
		/bin/echo "done"
	else
		/bin/echo "Le host $WILDCARD_ZONE existe déjà, il n'a pas été créé."
	fi
fi

#Créer un host supplémentaire?
/bin/echo -n "Créer un host en plus? [laisser vide pour non]: "
read ADDITIONAL_HOST

if [ -n $ADDITIONAL_HOST ]; then
	if ! host_exists $ADDITIONAL_HOST ; then

		/bin/echo "Creating a virtualhost for $ADDITIONAL_HOST..."
		/bin/echo -n "+ Adding $ADDITIONAL_HOST to /etc/hosts... "
		/bin/echo "$IP_ADDRESS	$ADDITIONAL_HOST" >> /etc/hosts
		/bin/echo "done"
	else
		/bin/echo "Le host $ADDITIONAL_HOST existe déjà, il n'a pas été créé."
	fi
fi
...
create_virtualhost $VIRTUALHOST "${DOC_ROOT_PREFIX}/${FOLDER}" $log $WILDCARD_ZONE $ADDITIONAL_HOST
/bin/echo "done"
